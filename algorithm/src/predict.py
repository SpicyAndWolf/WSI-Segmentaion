...
def predict(slide_folder, slide_name, isNormalized, patches_dir):
    # 确定dict_name
    config_type = 'DEFAULT'
    ...
    res_dir_name= slide_folder_name+"_"+slide_name+"_" + isNormalized
    ...
    batch_size = 64
    num_workers = 4
    dataset = PatchImageFolder(root=patches_dir, transform=transform)
...
def mergePatches(slide_folder, slide_name, df, patch_size, res_dir, isNormalized):
    # 确定dict_name
    config_type = 'DEFAULT'
    if isNormalized == "normalized":
        config_type = 'NORMALIZED'
