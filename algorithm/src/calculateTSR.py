...
def visualize_hotspot(res_dir, slide_name, grid, best_cy, best_cx, radius_grid):
    """
    可视化网格并标记热点区域。
    ...
    save_path = os.path.join(res_dir, hotspot_file_name)
    plt.savefig(save_path, bbox_inches='tight', pad_inches=0.1)
    plt.close(fig)
    return hotspot_file_name

def calculate_hotspot_tsr(dict_name, slide_folder, slide_name, res_dir, mpp, magnification, delta=1, diameter_mm=2):
    """
    计算指定WSI的热点区域TSR。
    ...
    if best_circle_y is not None and best_circle_x is not None:
        ...
        hotspot_file_name=visualize_hotspot(res_dir, slide_name, grid, best_circle_y, best_circle_x, radius_grid)
    else:
        logger.warning(f"No suitable hotspot found for {slide_name}.")

    return max_tsr, hotspot_file_name

def calculate_all_tsr(slide_folder, slide_name, res_dir, dict_name):
    """
...
def calculateTSR(slide_folder, slide_name, res_dir, isNormalized):
    """
    计算指定 WSI 的肿瘤间质比 (TSR)，直接使用降噪后的 prediction.csv。
    """
    # 加载配置文件
    ...
    # 计算 TSR
    tsr = calculate_all_tsr(slide_folder, slide_name, res_dir, dict_name)
    ...
    tsr_hotspot, hotspot_file_name = calculate_hotspot_tsr(dict_name, slide_folder, slide_name, res_dir, mpp, magnification)
