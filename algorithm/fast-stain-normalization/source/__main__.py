...
def single_transform(img_path, device, save_path, torch_normalizer, path, backend):
    ...
    try:
        ...
        cv2.imwrite(os.path.join(save_path, path), cv2.cvtColor(norm, cv2.COLOR_RGB2BGR))
    ...
    for subdir in os.listdir(img_path):
        ...
        for path in os.listdir(subdir_path):
            if path.split(".")[-1].lower() not in ["png", "jpg", "jpeg", "tif", "tiff"]:
                ...
                continue
            img = cv2.imread(os.path.join(subdir_path, path))
            if img is None:
                print(f"\nFailed to read image: {os.path.join(subdir_path, path)}")
                continue
...
def run_batch_parallel(reference_image_filename, img_path, out_path, cpu, parallel, workers, backend):
    ...
    # Iterate over subdirectories in img_path
    base_save_path = out_path
    for subdir in os.listdir(img_path):
        subdir_path = os.path.join(img_path, subdir)
        if not os.path.isdir(subdir_path):
